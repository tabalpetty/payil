...
1. Increase the fuzziness level in the keyword filter to accept more typos.
2. Introduce a hybrid search that unifies vector and keyword scoring.
...
1. Switch to a pure keyword search for all queries.
2. Increase the number of vector dimensions in embeddings.
3. Weight vector similarity higher than keyword scores in the custom scoring function.
...
1. Remove the vector component to prioritize keyword-only matches.
2. Lower the candidate result set size to only top N vector matches.
3. Use default scoring without any adjustments.
4. Increase keyword score contribution for rare or domain-specific terms.
...
1. Train a new embedding model on recent product descriptions.
2. Customize the hybrid scoring function to add a recency weight to candidate documents.
...
1. Reduce the vector dimension to minimize storage consumption.
2. Switch to a single global dense vector for each document.
3. Optimize index sharding and mapping for high query concurrency.
...
1. Disable keyword search to rely on better embeddings.
2. Increase the size of the vector candidate pool only.
3. Use a stop-words filter to ignore legal terminology.
4. Boost keyword term weighting for known legal phrases and combine with semantic ranking.
...
1. Adjust the similarity threshold in the vector search to retrieve more passages.
2. Implement a reranking model on the set of retrieved passages before sending to the Foundation Model.
...
1. Switch from OpenSearch to a custom-developed vector database.
2. Reduce the size of the candidate pool for initial retrieval.
3. Integrate a model-based reranking step after candidate retrieval.
...
1. Disable reranking and rely solely on the base retrieval order.
2. Parallelize both retrieval and reranking for all candidates, regardless of necessity.
3. Switch to a less accurate, lightweight retriever and remove reranking.
4. Limit reranking to only the top-N retrieved results rather than the whole candidate set.
...
1. The initial retriever always guarantees high relevance, so reranking is unnecessary.
2. Reranking was needed to reorder marginally relevant candidates for those queries.
...
1. It prevents candidates from the custom index from being included.
2. It only uses candidates from the highest-scoring retrieval source.
3. It ensures that candidates from all retrievers are compared and ordered by relevance for the user’s query.
...
1. The candidate retriever was disabled, so the reranker had nothing to process.
2. Adding any reranker always decreases relevance if not manually tuned.
3. Reranking is only beneficial when used with exact keyword matches.
4. The reranker was not fine-tuned on your domain or task, limiting its effectiveness.
...
1. The retriever failed to find relevant information due to poor indexing.
2. The reranker objective or features are not well aligned with the FM's downstream requirements.
...
1. Increase the number of retrievers in the pipeline.
2. Replace the reranking model with a rule-based approach immediately.
3. Examine the input features and training data used by the reranker.
...
1. Increase the vector store index size to include more data chunks.
2. Use Step Functions to schedule periodic vector index refreshes.
3. Switch to keyword-only search for initial retrieval.
4. Apply query decomposition using Lambda functions to break complex queries into smaller, manageable sub-queries.
...
1. Increase vector database read throughput.
2. Use Lambda to decompose questions and Step Functions to coordinate parallel retrieval.
...
1. Lower the semantic similarity threshold.
2. Expand the query to incorporate antonyms.
3. Apply query transformation using context-specific templates before retrieval.
...
1. Reduce the number of top-k retrieved results.
2. Switch to strict keyword search only.
3. Double the embedding vector dimension size.
4. Expand user queries using domain-specific synonym dictionaries.
...
1. Force all user queries to lower case before search.
2. Adopt query expansion to automatically generate and search with paraphrases.
...
1. Top-k retrieval limit is too low, requiring multiple retrieval passes.
2. Embedding model vector dimensions are excessively large.
3. Transformation with external function calls introduces processing overhead before retrieval.
...
1. Increase the number of retrieved results to top-50.
2. Expand queries using antonyms and unrelated synonyms.
3. Switch to single-field exact-match keyword search only.
4. Segment compound questions into individual sub-queries before retrieval.
...
1. Disable either expansion or decomposition entirely.
2. Implement deduplication logic after collecting all results from both strategies.
...
1. Increase the embedding batch size to process more documents in each run.
2. Set longer refresh intervals to minimize system load.
3. Implement real-time change detection to trigger immediate vector ingestion upon document addition.
...
1. Disable all scheduled jobs to avoid conflicts and rely only on manual ingestion triggers.
2. Recompute embeddings only on a monthly basis to ensure consistency.
3. Depend solely on the underlying document management system's access logs.
4. Enable event-driven ingestion pipelines and schedule regular refresh jobs.
...
1. Schedule hourly full re-indexing jobs for the entire document corpus.
2. Implement an incremental update pipeline triggered by CMS change events.
...
1. Increase the embedding model size to improve coverage.
2. Switch from batch to real-time embedding for all records.
3. Implement a reconciliation process to detect and enqueue unprocessed records.
...
1. Vector stores automatically inherit all authorization from source systems.
2. Source document permissions are always encoded in embeddings.
3. Embedding sensitive data protects against unauthorized access.
4. Embedding representations do not preserve or enforce original document-level authorization.
...
1. Implementing drift monitoring with diagnostics for change detection.
2. Ignoring drift unless end users report obvious retrieval failures.
...
1. Apply a single hourly batch job for all sources, regardless of their update cadence.
2. Force a global embedding refresh on every update event from any source.
3. Configure incremental update pipelines per source and combine periodic global audits.
...
1. Depend on vector similarity score only for determining the most relevant documents.
2. Schedule daily index rebuilds and alert users for potential staleness.
3. Hard-code a list of the newest document IDs at application startup.
4. Integrate a last-updated timestamp into both the embedding and the retrieval filter logic.
...
1. Let each FM client directly query the vector database using its specific native driver.
2. Implement a serverless retrieval API layer providing a function interface for vector search and policy enforcement.
...
1. Send direct SQL queries from the FM to the vector database, logging only FM prompt and response.
2. Query the vector store from the client, then forward retrieved results through an API Gateway endpoint to the FM.
3. Build the retrieval API as an AWS Lambda function with CloudWatch logging of all queries, and invoke it through Bedrock’s function calling/MCP interface.
...
1. Require each FM to use its own bespoke retrieval method and store integration logic within model-specific wrappers.
2. Allow FMs to connect directly to various vector databases, configuring authentication in each FM separately.
3. Deprecate retrieval augmentation in favor of prompt-only workflows managed by application code.
4. Implement a Model Context Protocol (MCP)–compliant retrieval client that all FMs use, abstracting storage and logic.
...
1. Send raw unstructured text blobs from the vector store to the FM, relying on prompt templates for structure.
2. Wrap retrieved chunks in a well-defined JSON schema via a function interface for downstream FM integration.
...
1. Hard-code all retrieval flows inside existing Bedrock-specific adapters.
2. Design the vector store API using bespoke endpoints for each FM’s requirements.
3. Adopt the Model Context Protocol (MCP) client or standardized function interface in all RAG applications.
...
1. Client-side calls maximize security by eliminating server involvement in data handling.
2. Direct vector store access from browsers enhances cache performance and reduces API costs.
3. Such a design is needed for fastest real-time document indexing.
4. It exposes internal data sources and bypasses retrieval-layer validation, risking data leaks and inconsistent application logic.
...
1. Restart the application and clear the vector store cache to eliminate old vectors.
2. Analyze the quality of input query embeddings and verify their alignment with the retrieval corpus.
...
1. Increase the FM temperature parameter to see if it speeds up responses.
2. Switch from semantic to keyword search to check if results are faster.
3. Instrument step-level latency and collect metrics at each major retrieval component.
...
1. The retrieval query is being truncated due to token limits enforced by the FM.
2. The RAG pipeline is using an outdated FM API version.
3. An incorrect access policy is preventing retrieval of relevant documents.
4. Chunk size and overlap parameters are improperly tuned, leading to fragment loss in the embedding step.
...
1. Decrease the retrieval top-k value to streamline the context injection.
2. Re-embed all documents in the knowledge base using the new embedding model.
...
1. Increase the FM's context window to allow more input text.
2. Switch to a different embedding model for all queries.
3. Compare current and prior chunking logic to identify changes in document segmentation.
...
1. The FM model itself must be retrained to reflect the most recent updates.
2. Reranking models in Bedrock are not effective with updated semantic vectors.
3. API request timeouts caused the retrieval pipeline to occasionally fall back to default answers.
4. The vector store synchronization or refresh pipeline has failed, so the search mechanism is reading stale embeddings.
...
1. Increase the document retrieval top-k value for broader data input.
2. Optimize context assembly by setting a maximum context size and prioritizing relevant chunks.
...
1. Use a single-item table where the conversation state for all users is stored in a large JSON blob.
2. Create a table with PartitionKey as session ID and SortKey as interaction index.
3. Use a table with PartitionKey as user ID and SortKey as a timestamp for each message.
...
1. Rely only on the detected intent from Comprehend and ignore language detection.
2. Convert all incoming text to English using translation APIs before passing to Comprehend.
3. Add an additional timestamp field to DynamoDB for each interaction.
4. Check the detected language from Comprehend and branch logic when the confidence is low or language is unsupported.
...
1. Restrict system usage to only the English language.
2. Check Comprehend’s list of supported languages and add a fallback response when an unsupported language is detected.
...
1. Rely on Comprehend to return valid intent for all input regardless of language.
2. Store the region code in DynamoDB and use it to determine language support without running language detection.
3. Integrate language detection from Comprehend, verify detection confidence, and provide custom error handling when language is unsupported.
...
1. Run a table scan and filter by user ID after retrieval.
2. Store all conversation messages in a large JSON attribute and retrieve it for each request.
3. Use a session ID as the partition key and ignore the timestamp in results.
4. Query the table using the user ID as the partition key and sort the results by the timestamp SortKey in descending order, limiting the number of results.
...
1. Store a serialized conversation object in a single DynamoDB item and parse it on each message.
2. Use AWS Step Functions to orchestrate the conversation flow, invoking Lambda for stateful transitions based on user input and tracked intent.
...
1. Partition conversation state by device ID, requiring users to re-authenticate on each device.
2. Use a non-persistent, device-local cache to store conversation state.
3. Identify and partition conversation records by user ID rather than device or session ID, ensuring history is accessible regardless of device.
...
1. Retrain the language model using more recent Spanish data.
2. Switch to a third-party service for all Spanish language utterances.
3. Disable support for Spanish until further notice.
4. Verify using the Comprehend documentation that Spanish is still a supported language and review service health in the intended region.
...
1. Provide the FM with additional negative examples.
2. Specify explicit output constraints and expected field names.
...
1. Switch to a different FM provider that prefers numbered lists.
2. Shorten the prompt by removing prior context.
3. Edit the prompt to explicitly require a numbered, ordered list and add an example.
...
1. Only provide more example summaries in the prompt.
2. Switch the FM to a smaller parameter count for more concise answers.
3. Lower the response temperature to its minimum value.
4. Integrate feedback loops that rate and filter summaries based on relevance.
...
1. Reduce FM inference temperature to zero.
2. Parameterize critical parts of the prompt and standardize formatting.
...
1. Increase the inference temperature above default.
2. Remove all output constraints so the FM is free to elaborate.
3. Instruct the FM in the prompt to provide step-by-step reasoning using chain-of-thought.
...
1. Switch to a prompt without any context and rely on the FM's pretraining.
2. Use as many diverse context sources as possible for each prompt regardless of length.
3. Manually curate each FM response after generation.
4. Introduce a feedback loop to rank outputs and adjust prompts based on factual correctness.
...
1. Let users edit free-text prompts in the configuration file.
2. Expose prompt parameters such as tone and format as input variables in a parameterized template.
...
1. Only continue prompt iteration cycles until outputs are satisfactory.
2. Switch to a more powerful FM with more training data.
3. Implement application-level feedback and validation of FM responses.
...
1. Relying solely on version identifiers in template names.
2. Using IAM roles to restrict access to the prompt builder UI.
3. Storing prompt templates in a shared Google Drive folder.
4. Enabling AWS CloudTrail to log all prompt template operations.
...
1. Allow all team members editor access to the production prompt repository.
2. Configure approval workflows and restrict write permissions on production prompt repositories.
...
1. Rely on model inference logs without prompt context.
2. Manually review deployed prompt template files for each incident.
3. Enable event logging (such as AWS CloudTrail and CloudWatch Logs) for all prompt management operations.
...
1. Allow direct manual edits in production after development testing.
2. Use a shared repository and trust developers to only make approved changes.
3. Configure templates to allow dynamic overwrites based on latest test run.
4. Require template approval and enforce repository immutability until passed checks are complete.
...
1. Replace the broken prompt with an ad hoc hot patch.
2. Maintain strict versioning of all prompt templates with approval checkpoints and rollback procedures.
...
1. Allow code reviews to implicitly substitute for formal approvals.
2. Expect engineering managers to remember which prompts they approved.
3. Integrate approval metadata and approver identity storage in the prompt repository.
...
1. Delete all legacy prompts lacking audit data.
2. Move all templates to a private folder accessible only to platform admins.
3. Disable prompt version control to avoid confusion.
4. Enforce metadata labeling for all templates, including deployment date and usage status.
...
1. Rely on user feedback and post-deployment bug reports to catch prompt regressions in production.
2. Deploy a continuous integration (CI) pipeline that triggers regression tests on prompt changes, using test cases for all supported clients.
...
1. Focus only on the most common user queries to improve QA test efficiency.
2. Increase the number of manual reviews for each prompt change.
3. Expand regression test coverage to include edge case prompts and use Step Functions for orchestrating scenario-based test workflows.
...
1. Increase the number of static prompt test cases for high-frequency inputs.
2. Switch to entirely manual prompt validation before deployment.
3. Reduce the frequency of deployments to decrease the chance of regressions.
4. Expand regression test cases to cover edge scenarios and rare usage patterns.
...
1. Creating a feedback loop using output format specifications
2. Configuring a conditional branching node based on model output
...
1. Implement an unconditional prompt chaining sequence of three steps
2. Add a feedback loop for the end-user at every step regardless of confidence
3. Implement a branching node that evaluates the model’s confidence score
...
1. Build a single prompt template that accepts all raw user input and output in one step
2. Configure branching logic exclusively for output format enforcement
3. Deploy multiple Prompt Flows, each with its own preprocessing script and isolated evaluation stage
4. Use pre-processing and post-processing steps with reusable component nodes connected to the main prompt chain
...
1. Mandatory sequential execution of all processing nodes
2. Conditional branching driven by evaluation of the input type
...
1. Use a single prompt that incorporates all possible follow-up instructions using if-else logic in prompt text
2. Execute both follow-up reasoning prompts in parallel and merge their outputs
3. Add a branching component that examines the product type and routes to specific sub-prompts
...
1. A missing branching node that should route to alternative summarizers
2. All steps are running in required sequence, so there is no issue
3. Summarization must always immediately follow input, not other prompts
4. The flow fails to properly pass outputs between nodes, breaking the chain
...
1. It reduces total latency by always executing all sub-steps in parallel
2. It enables granular visibility, error handling, and modular updates of each business rule and transformation step
...
1. Replace the downstream prompt with an integrated API fetch prompt
2. Run the API call separately outside of Prompt Flows and manually inject results
3. Insert a function component node between the two prompt nodes to call the external API
...
1. Post-processing is only permitted after a branching node
2. Pre-processing overwrites all downstream variables by default
3. The flow is invalid without a merge node
4. The output variable from the language model node is not properly mapped to the post-processing node
...
1. Increase the burst limit in API Gateway.
2. Switch to a REST API Gateway instead of HTTP API.
3. Increase Lambda provisioned concurrency for the function.
...
1. Custom domain names and usage plans
2. API logging and caching
3. Enable WAF and custom request headers
4. Request validation and throttling limits
...
1. Require an API key and usage plan
2. Enable body validation in API Gateway method request validation
...
2. Set up an API Gateway usage plan
3. Provisioned concurrency for the Lambda function
...
1. Amazon Bedrock model parameters reference
2. Lambda maximum event size
3. API Gateway logging best practices
4. API Gateway endpoint quotas and limits
...
1. Use Lambda handler code to parse and reject malformed JSON.
2. Enable request validation on the API Gateway integration method.
...
1. Configure a scheduled CloudWatch event to invoke the function regularly.
2. Enable dead-letter queues for the Lambda function.
3. Configure provisioned concurrency at a suitable value for the Lambda function.
...
1. Lambda’s memory allocation is too low for handling the payload.
2. Bedrock’s individual model input parameter size is being exceeded.
3. Downstream service concurrency limits have been hit.
4. API Gateway request and response payload limits are being exceeded.
...
1. SQS automatically deletes messages after the first processing attempt.
2. The Lambda function failed its execution, so the SQS messages became visible again and were retried.
...
1. Direct invocation of Bedrock from each microservice
2. A single Lambda function polling an SQS queue
3. Step Functions' workflow orchestration for microservice integration
...
1. SQS FIFO queue is causing delays due to sequencing.
2. Bedrock FM API throttling is causing Lambda failures.
3. Step Functions wait tasks are holding messages intentionally.
4. Lambda’s concurrency isn’t high enough to keep up with the arrival rate.
...
1. Increase Lambda function memory for document extraction.
2. Add Catch and Retry policies in the Step Functions state machine for the document extraction step.
...
1. API Gateway calls Bedrock FM synchronously within a Lambda.
2. Step Functions synchronous Express workflow.
3. API Gateway triggers SQS, then a Lambda function processes SQS messages for Bedrock inference.
...
1. Dedicate a single EC2 instance to run multiple inference jobs as background threads.
2. Invoke API Gateway synchronous endpoints from a central orchestrator.
3. Use Step Functions synchronous state machines.
4. Use SQS with Lambda for parallel job invocation and auto scaling.
...
1. Decrease Lambda timeout to enforce faster responses.
2. Enable Lambda response streaming in the Lambda function configuration.
...
1. Deploy a traditional REST API that collects the full FM response before sending it to the client.
2. Use API Gateway with an SQS integration for buffering responses.
3. Configure API Gateway HTTP API with chunked transfer encoding and integrate with a Lambda function using Lambda response streaming.
...
1. Switch to using API Gateway REST API instead of HTTP API.
2. Increase the connection timeout for the client.
3. Enable synchronous invocation for the Lambda function.
4. Review the Lambda function's response streaming configuration.
...
1. Implement WebSocket connections for both server and client communication.
2. Rely on HTTP polling at short intervals.
3. Use Server-Sent Events (SSE) to stream server updates to the client.
...
1. API Gateway REST APIs provide built-in chunked transfer encoding for Lambda integrations.
2. Chunked transfer encoding guarantees delivery of all chunks even if a client disconnects early.
3. Chunked transfer encoding requires all responses to be transformed into string literals on the server.
4. Chunked transfer encoding delivers incremental responses to connected HTTP clients and may require clients to handle partial JSON fragments until completion.
...
1. The Lambda function memory allocation is too low for the response body.
2. Streaming with chunked transfer encoding delivers partial data as chunks, so clients may see incomplete JSON until the full stream is complete.
...
1. Set the 'conversation' flag to false to toggle back to stateless requests.
2. Increase model maximum token parameter for conversation mode.
3. Format each conversation turn as a separate JSON object in the request payload.
...
1. Double the number of parallel requests to increase total throughput.
2. Switch to a round-robin DNS approach for endpoint selection.
3. Decrease the SDK retry count to minimize repeated requests.
4. Implement exponential backoff between retries to space requests appropriately.
...
1. Disable request retries to ensure low latency.
...
1. Security of the API is compromised.
2. API Gateway rate limits are bypassed.
3. Higher reliability at the cost of increased response times.
...
1. Examine whether Lambda provisioned concurrency is set too low.
2. Investigate if the Bedrock model endpoint is unhealthy.
3. Review IAM policies attached to Lambda for resource constraints.
4. Check if API Gateway rate/burst limits are lower than Lambda's processing capacity.
...
1. The API Gateway endpoint is misconfigured for HTTPS.
2. The client is retrying failed requests without exponential backoff.
...
1. Disable exponential backoff retries to reduce duplicated calls.
2. Switch all request-response interactions from synchronous to asynchronous.
3. Instrument API Gateway and client with end-to-end timing and trace logging.
...
1. Reducing retry count below five leads to loss of all resilience.
2. Retry settings are required to be equal across all AWS SDKs.
3. Model accuracy is affected by the retry count.
4. Unreasonably high retry count can cause cascading retries under stress, worsening overload.
...
1. Deploy the application only in a single Region with the FM and rely on retry logic.
2. Implement cross-Region failover with Bedrock Cross-Region Inference and a graceful degradation mechanism.
...
1. Increase the FM API retry attempts to the maximum allowed.
2. Use exponential backoff in the retry settings.
3. Implement a fallback workflow using Step Functions to return canned responses when FM inference fails.
...
1. A single failed request to the FM API.
2. Short bursts of high latency under the threshold.
3. Exceeding the rate limit for one second on the FM API.
4. Repeated FM API response timeouts beyond a threshold over a sliding window.
...
1. Suppress all error messages and return null responses.
2. Add a fallback path that delivers a contextually appropriate summary or previously cached answer.
...
1. Configure the circuit breaker to open after a single error event.
2. Disable the circuit breaker in favor of constant retry.
3. Set thresholds based on empirical error rates over a realistic monitoring window.
...
1. Disable the SDK retry mechanism entirely.
2. Set a very high retry count with no limit to maximize success chance.
3. Add extra sleep between retries instead of using circuit breakers.
4. Implement a circuit breaker that halts retries and invokes fallback after repeated failures.
...
1. Rely purely on exponential backoff with aggressive retries.
2. Configure a circuit breaker in the FM inference state to trigger a fallback after repeated failures.
...
1. Add more concurrent requests to Bedrock to test for service availability.
2. Have users submit support tickets if their queries fail.
3. Introduce circuit breaker logic and automated fallback to cached or approximate responses.
...
1. Filter traces by request metadata and export raw trace data to Amazon S3 for offline analysis.
2. Configure custom annotations in your application code to tag traces with model names.
3. Use X-Ray Insights to detect anomalies in Lambda cold start times.
4. Use the X-Ray service map to drill down into individual segment traces and analyze duration outliers.
...
1. Log Streams filters grouped by runtime environment
2. Multi-metric line graphs with widgets sorted by intent and time
...
1. Trace List grouped by API Gateway execution time
2. CloudWatch Dashboard with transaction counters
3. Service Map with highlighted error nodes and trace paths
...
1. X-Ray service map error percent coloring
2. CloudWatch alarm actions
3. CloudWatch default metric graphs
4. X-Ray annotations and filter expressions
...
1. Increase the polling frequency of infrastructure-level error counters.
2. Add custom widgets that calculate business KPIs (e.g., revenue impact, orders affected) using metric math or Lambda metric ingestion.
...
1. Use alarm status summaries to identify error timing across services.
2. Filter logs for HTTP 504 entries and cross-reference in the dashboard.
3. Select problematic traces from the trace list and examine segment timelines for retry and timeout patterns.
...
1. Rely exclusively on passing standard prompt test cases from the main business workflow.
2. Increase logging for failed prompt outputs without modifying the test process.
3. Reduce deployment frequency to allow more time for post-deployment monitoring.
4. Expand the regression test suite to include region-specific and edge case prompts drawn from real-world usage.
...
1. Schedule quarterly manual reviews of chat transcript outputs for anomalies
2. Introduce continuous automated regression testing in the deployment pipeline covering all critical frontend use cases
...
1. Only test prompts during off-peak hours to avoid impacting live customers
2. Increase the minimum passing score for model response relevance in the current tests
3. Continuously update regression test datasets to include emerging query patterns and edge user behaviors
...
1. Increase the allowed API payload size in the Bedrock console.
2. Reduce model temperature to minimize hallucinations.
3. Disable response streaming to maximize context retention.
4. Implement dynamic prompt truncation based on model token limit parameters.
...
1. The model's temperature is too high; decrease temperature to improve answer accuracy.
2. Conversation plus user input exceeded the model's token context window; introduce summarization or truncation of earlier conversation turns.
...
1. Check for incorrect AWS credentials in API Gateway.
2. Increase CloudWatch log verbosity for output responses.
3. Compare total token count (system + user input) to the FM model's maximum context window per request.
...
1. Adding an exponential backoff retry handler to absorb intermittent errors.
2. Forcing API requests to be synchronous only.
3. Appending a static system prompt to all requests for improved validation.
4. Token-counting each constructed prompt and enforcing the model’s documented token limit.
...
1. Pass all user input directly and handle failures as they occur.
2. Integrate language-specific tokenization and maximum-token enforcement for each supported Bedrock FM as part of the client library.
...
1. Download all raw trace JSON data for local log analysis.
2. Configure CloudWatch metric alarms for error rates on all Lambda functions.
3. Use the X-Ray service map to visualize and filter traces by error status between microservices.
...
1. Export all technical metrics to an external BI tool for full business visibility.
2. Rely solely on built-in AWS service dashboards, as these automatically include business indicators.
3. Set up alerts for model latency, emailing PDF reports of metrics to stakeholders weekly.
4. Add custom widgets that combine CloudWatch metrics (for latency) and business KPIs (from custom metrics) into a single dashboard.
...
1. Send the user engagement metric to Amazon SNS and link CloudWatch dashboards by alarm name.
2. Define a custom metric for user engagement and add it as a separate graph alongside FM latency and error metrics in the same CloudWatch dashboard.
