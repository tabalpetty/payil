...
CLAIM_REPAIR_TAIL_MAX_RATIO = 0.15
HUMAN_SOURCE_REVIEW_TAIL_MAX_RATIO = 0.05
...
def claim_source_verification_path(day: int) -> Path:
...
def claim_verification_counts(day: int) -> Counter[str]:
    path = claim_source_verification_path(day)
    if not path.exists():
        return Counter({"missing": 1})
    data = json.loads(path.read_text(encoding="utf-8"))
    return Counter(item.get("verdict", "missing") for item in data.get("items", []))
...
def run_gates(day: int, target: int) -> dict[str, Any]:
    ...
    claim_counts = claim_verification_counts(day)
    claim_total = sum(claim_counts.values())
    llm_repair_tail_count = claim_counts.get("unresolved", 0)
    human_review_tail_count = source_status.get("needs-human-source-review", 0)
    llm_repair_tail_ratio = safe_ratio(llm_repair_tail_count, claim_total)
    human_review_tail_ratio = safe_ratio(human_review_tail_count, len(items))
    tail_threshold_breaches = []
    if llm_repair_tail_ratio > CLAIM_REPAIR_TAIL_MAX_RATIO:
        tail_threshold_breaches.append(
            f"llm_repair_tail {llm_repair_tail_count}/{claim_total} "
            f"({llm_repair_tail_ratio:.1%}) > {CLAIM_REPAIR_TAIL_MAX_RATIO:.0%}"
        )
    if human_review_tail_ratio > HUMAN_SOURCE_REVIEW_TAIL_MAX_RATIO:
        tail_threshold_breaches.append(
            f"human_review_tail {human_review_tail_count}/{len(items)} "
            f"({human_review_tail_ratio:.1%}) > {HUMAN_SOURCE_REVIEW_TAIL_MAX_RATIO:.0%}"
        )
    ...
    gates = {
        ...
        "automation_tail_gate": "pass" if not tail_threshold_breaches else "fail",
    ...
    return {
        ...
        "claim_verification_counts": dict(claim_counts),
        "automation_tail_thresholds": {
            "llm_repair_tail_max_ratio": CLAIM_REPAIR_TAIL_MAX_RATIO,
            "human_source_review_tail_max_ratio": HUMAN_SOURCE_REVIEW_TAIL_MAX_RATIO,
        },
        "automation_tail_metrics": {
            "llm_repair_tail_count": llm_repair_tail_count,
            "llm_repair_tail_ratio": round(llm_repair_tail_ratio, 4),
            "human_review_tail_count": human_review_tail_count,
            "human_review_tail_ratio": round(human_review_tail_ratio, 4),
        },
        "automation_tail_threshold_breaches": tail_threshold_breaches,
...
def render_report(result: dict[str, Any]) -> str:
    ...
    lines.extend(
        [
            ...
            f"- Claim verification status: `{result['claim_verification_counts']}`",
            ...
            "",
            f"- Thresholds: `{result['automation_tail_thresholds']}`",
            f"- Metrics: `{result['automation_tail_metrics']}`",
            f"- Breaches: `{result['automation_tail_threshold_breaches'] or 'none'}`",
            "",
            "## Cost Summary",
            "",
