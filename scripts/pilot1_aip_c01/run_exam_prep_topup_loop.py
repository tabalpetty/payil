...
def main() -> int:
    ...
    for iteration in range(1, args.max_iterations + 1):
        ...
        run_command(
            [sys.executable, "scripts/pilot1_aip_c01/normalize_exam_prep_questions.py", "--day", str(args.day)],
            logger,
            "normalize",
        )
        run_command(
            [sys.executable, "scripts/pilot1_aip_c01/review_exam_prep_questions.py", "--day", str(args.day)],
            logger,
            "review",
        )
        ...
        for topic, deficit in sorted(shorts.items()):
            ...
            command = [
                sys.executable,
                "scripts/pilot1_aip_c01/generate_exam_prep_questions.py",
                "--day",
                str(args.day),
                "--top-up-topic",
                topic,
