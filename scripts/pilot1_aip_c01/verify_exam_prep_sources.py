...
def review_checks_json_path(day: int) -> Path:
...
def normalized_claim_text(item: str) -> str:
    return " ".join(item_claim_text(item).split())


def claim_text_hash(item: str) -> str:
    return hashlib.sha256(normalized_claim_text(item).encode("utf-8")).hexdigest()


def atomic_claims_from_item(item: str) -> list[dict[str, str]]:
    block = extract_block(item, "Atomic claims:", ["Generation notes:", "Normalization warnings:", "### "])
...
def ungrounded_source_trace(trace: str) -> bool:
    normalized = " ".join(trace.strip().split())
    ...
    if ANY_URL_RE.search(normalized):
        return True
    if local_paths_from_trace(normalized):
        return source_trace_has_extra_prose(normalized)
    return True


def source_trace_has_extra_prose(trace: str) -> bool:
    residue = OFFICIAL_AWS_DOC_URL_RE.sub("", trace)
...
def verify_item(
    ...
    *,
    ...
    elif ungrounded_source_trace(trace):
        reasons.append("source_trace is not a resolvable official AWS URL, local docs path, or explicit NONE/source_trace_needed marker")
    ...
    elif ledger_record.get("claim_text_hash") != claim_text_hash(item):
        reasons.append("claim verification ledger record does not match current item content")
...
def initialize_claim_ledger(day: int, items: list[str]) -> None:
    ...
    for item in items:
        fields = parse_fields(item)
        item_id = fields.get("item_id", "unknown")
        current_hash = claim_text_hash(item)
        current_claim = normalized_claim_text(item)
        item_atomic_claims = atomic_claims_from_item(item)
        if (
            item_id in existing
            and existing[item_id].get("claim_text_hash") == current_hash
            and existing[item_id].get("verdict") == "verified"
        ):
        ...
        records.append(
            {
                "item_id": item_id,
                "learning_unit": fields.get("learning_unit", "unknown"),
                "claim_text_hash": current_hash,
                "claim_text_excerpt": current_claim[:500],
                "verdict": "unresolved",
                "claims": claims
            }
        )
...
def update_review_output_checks(day: int, counts: Counter[str]) -> None:
    status_counts = {
        "source-verified": counts["source-verified"],
        "needs-human-source-review": counts["needs-human-source-review"],
    }
    json_path = review_checks_json_path(day)
    if json_path.exists():
        data = json.loads(json_path.read_text(encoding="utf-8"))
        data["source_trace_status_counts"] = status_counts
        json_path.write_text(json.dumps(data, indent=2) + "\n", encoding="utf-8")

    md_path = review_checks_path(day)
    if md_path.exists():
        text = md_path.read_text(encoding="utf-8")
        replacement = (
            "| Final source verification | pending | "
            f"source_trace_status_counts={status_counts}; "
            f"see `{day_slug(day)}-source-verification-report.md` |"
        )
        text = re.sub(
            r"(?m)^\| Final source verification \|.*$",
            replacement,
            text,
            count=1,
        )
...
def main() -> int:
    ...
    counts = Counter(result["status"] for result in results)
    update_review_output_checks(args.day, counts)
